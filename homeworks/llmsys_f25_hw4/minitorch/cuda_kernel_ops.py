...
class CudaKernelOps(TensorOps):
    ...
    def attn_softmax_bw(out_grad: Tensor, soft_inp: Tensor):
      #   BEGIN ASSIGN4_1_2
      batch_size, nhead, from_len, soft_len = soft_inp.shape
      rows = batch_size * nhead * from_len
      stream = torch.cuda.current_stream().cuda_stream
      print(f"Softmax len {soft_len}")
      ...
      print(f"Returned out grade shape {out_grad.shape}")
    ...
    def layernorm_fw(inp: Tensor, gamma: Tensor, beta: Tensor):
      ...
      ln_vars = inp.zeros((seq_len,))
      ln_means = inp.zeros((seq_len,))

      lib_layernorm.launch_layernorm(
        ln_res._tensor._storage,
        ln_vars._tensor._storage,
        ln_means._tensor._storage,
        inp._tensor._storage,
